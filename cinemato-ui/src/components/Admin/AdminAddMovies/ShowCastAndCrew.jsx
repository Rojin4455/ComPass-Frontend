...
.map((member, index) => (
  <div key={index} className="text-center"> {/* Use only index */}
    <img
    ...
    <p className="text-sm text-gray-500">{member.character}</p>
        ...
        {castAndCrew.cast.length > 18 && !showAllCast && (
            <div className="mt-20">
        <CiCircleMore onClick={() => setShowAllCast(true)} className='' size={30}/>
            </div>
        )}
        {showAllCast &&(

        <div className="mt-20">
        <HiMiniArrowLeft onClick={() => setShowAllCast(false)} className='' size={30}/>
            </div>
          )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-12 gap-6">
        ...
        {castAndCrew.crew.length > 18 && !showAllCrew && (
            <div className="mt-14">
            <CiCircleMore onClick={() => setShowAllCrew(true)} className='' size={30}/>
                </div> 
        )}
        {showAllCrew &&(

        <div className="mt-14">
            <HiMiniArrowLeft onClick={() => setShowAllCrew(false)} className='' size={30}/>      
                </div>
          )}
