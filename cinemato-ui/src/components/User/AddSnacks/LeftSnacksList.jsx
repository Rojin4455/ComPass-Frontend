...
function LeftSnacksList({ categories, snacks }) {
    ...
    return (
        <div className="flex flex-col items-center space-y-4">
            ...
            <h1 style={{fontFamily:"Montserrat"}} className="text-2xl font-semibold">Grab a </h1>
            <h1 style={{fontFamily:"Montserrat"}} className="text-2xl font-semibold text-yellow-600">Bite!</h1>
            </div>
            ...
            <div className="flex space-x-2">
                {categories.map((category, index) => (
                    <div
                        ...
                        className={`px-2 py-0.5 border border-gray-400  rounded-full cursor-pointer text-xs ${category.name === selectedCategory ? "bg-secondary text-gray-700" : "text-gray-600 hover:bg-secondary hover:text-gray-600"
                            }`}
                ...
                return (
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-hidden">
                            <h4 className="text-lg font-semibold text-gray-800 truncate">{snack.name}</h4>
                            <p className="text-sm text-gray-500 truncate w-full overflow-hidden">{snack.description}</p>
                            <p className="text-md font-bold text-gray-700 mt-1">₹{parseFloat(snack.price).toFixed(2)}</p>
                        </div>

                        {isAdded && quantity > 0 ? (
                            <div className="flex items-center space-x-2">
                                <CiCircleMinus size={25} onClick={() => decrement(snack.id)} className="text-primary cursor-pointer" />

                                <span className="text-md font-semibold text-gray-800">{quantity}</span>

                                <CiCirclePlus size={25} onClick={() => increment(snack.id)} className="text-primary cursor-pointer" />
                            </div>
                        ) : (
                            <button
